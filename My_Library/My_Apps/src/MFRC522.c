...
void Write_MFRC522(u_char addr, u_char val) {
  //uint32_t rx_bits;
	  u_char addr_bits = (((addr<<1) & 0x7E));  // to set the 0th and 7th bits to 0 => enable writing
    spi_write(addr_bits, val);
}
//-----------------------------------------------
/*
...
u_char Read_MFRC522(u_char addr) {
  //uint32_t rx_bits;
  u_char rx_bits;
  u_char addr_bits = (((addr<<1) & 0x7E) | 0x80); // to set the 0th and 7th bits to 0 => enable reading
  rx_bits = spi_read(addr_bits);
